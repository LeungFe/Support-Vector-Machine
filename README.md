...
其中$\phi(x_{i})^{T}\phi(x_{j})$是$x_{i}$与$x_{j}$映射到特征空间之后的内积，由于特征空间维数比较高，甚至无穷维，因此直接计算$\phi(x_{i})^{T}\phi(x_{j})$通常非常困难，所以，引入核函数来解决这个问题。$x_{i}$与$x_{j}$在特征空间的内积等于它们在原始空间中通过核函数计算的结果，这样就可以避免在高维甚至无穷维特征空间中做内积，减少计算的复杂度。
